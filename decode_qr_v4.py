#!/usr/bin/env python3
"""
QR 码解码拼接工具 v4（Mac 端）— 多引擎识别 + 多进程加速 + v2 载荷格式

v4 仅解码 gen_terminal_qr_v4.py 生成的 v2 格式 QR：
    V2:FNAMEB32:IDX/TOTAL:TAG:DATAB32

历史 v1 载荷请继续使用 decode_qr_v3.py。v4 遇到非 V2 前缀的 QR 会静默忽略。
...
用法：
    python3 decode_qr_v4.py <截图目录> [输出目录]
    python3 decode_qr_v4.py <截图目录> [输出目录] -j4    # 指定 4 进程

示例：
    python3 decode_qr_v4.py ./screenshots/
...
def _b32_unpad(s: str) -> bytes:
    """恢复 base32 padding 后解码"""
    pad = '=' * (-len(s) % 8)
...
def parse_qr_text(text):
    ...
    返回 (fname, idx, tot, data, tag) 或 None（非 v2 / 格式错误）。
    """
    if not text.startswith('V2:'):
        return None

    # split 最多 5 段，避免 DATAB32 里不会出现但防御性处理
    parts = text.split(':', 4)
    if len(parts) != 5:
        return None

    _, fname_b32, header, tag, data = parts
    if tag not in ('X', 'Z'):
        return None

    try:
        fname = _b32_unpad(fname_b32).decode('utf-8')
    except Exception:
        return None

    try:
        idx_str, tot_str = header.split('/')
        idx, tot = int(idx_str), int(tot_str)
    except ValueError:
        return None

    return (fname, idx, tot, data, tag)
...
def reassemble_file(fname, chunks, total, output_dir, tag):
    """将单个文件的所有分片拼接、base32 解码、解压、写入"""
...
def decode_all(scan_dir, output_dir, num_workers, manifest_override=None):
    ...
    print(f"📐 载荷格式: v2 (Base32 / alphanumeric)")
    print()
    ...
    try:
        for fpath, texts in results_iter:
            ...
            for text in texts:
                ...
                fname, idx, tot, data, tag = parsed
                last_fname, last_idx, last_tot = fname, idx, tot
                recognized += 1

                if fname not in file_groups:
                    file_groups[fname] = {'total': tot, 'chunks': {}, 'tag': tag}
    ...
    for fname in sorted(file_groups.keys()):
        ...
        print(f"\n  📄 {fname} ({len(chunks)}/{tot} 片, tag={tag})")

        if reassemble_file(fname, chunks, tot, output_dir, tag):
            success += 1
        else:
            fail += 1
    ...
    # ==========================================
    # 基于 Manifest / 哨兵 / 手动清单 检查整文件丢失
    # ==========================================
    manifest_path = os.path.join(output_dir, "_manifest.txt")
    manifest_source = None

    if manifest_override and os.path.exists(manifest_override):
        manifest_source = manifest_override
        print("-" * 55)
        print(f"📋 使用手动指定的清单文件: {manifest_override}")
    elif os.path.exists(manifest_path):
        manifest_source = manifest_path

    meta_filenames = {"_manifest.txt", "_manifest_count.txt"}

    if manifest_source:
        print("-" * 55)
        print("🔍 执行全局文件完整性校验 (Manifest)")
        try:
            with open(manifest_source, "r", encoding="utf-8") as mf:
                expected_files = set(line.strip() for line in mf if line.strip())

            actual_files = set(file_groups.keys()) - meta_filenames
        ...
        sentinel_path = os.path.join(output_dir, "_manifest_count.txt")
        if os.path.exists(sentinel_path):
            ...
            try:
                ...
                if content.startswith("N="):
                    expected_count = int(content[2:])
                    actual_count = len(set(file_groups.keys()) - meta_filenames)
                    if actual_count < expected_count:
                        print(f"  🚨 文件数量不符: 期望 {expected_count} 个，实际收到 {actual_count} 个")
                        print(f"     有 {expected_count - actual_count} 个文件完全丢失，但无法确定具体文件名")
                        print(f"     建议: 使用 --manifest 选项手动指定文件清单进行精确校验")
                    else:
                        print(f"  ✅ 文件数量校验通过: {actual_count}/{expected_count}")
                else:
                    print(f"  ⚠️ 哨兵文件格式无效: {content}")
            except (ValueError, IOError) as e:
                print(f"  ⚠️ 读取哨兵文件时发生错误: {e}")
        else:
            if len(file_groups) > 1:
                print("-" * 55)
                print("  ⚠️ 未找到 _manifest.txt 和 _manifest_count.txt。")
                print("     如果源端生成了清单，这说明清单的二维码已完全丢失，无法进行整文件丢失校验。")
                print("     建议: 使用 --manifest 选项手动指定文件清单进行校验")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 decode_qr_v4.py <截图目录> [输出目录] [-jN] [--manifest <文件清单路径>]")
        print()
        print("注意: v4 仅解码 gen_terminal_qr_v4.py 的 V2: 前缀载荷。")
        print("      历史 v1 QR 请使用 decode_qr_v3.py。")
        sys.exit(1)
    ...
    while i < len(args):
        ...
        if arg == '--manifest':
            if i + 1 < len(args):
                manifest_override = args[i + 1]
                i += 2
                continue
            else:
                print("⚠️  --manifest 需要指定文件路径")
                sys.exit(1)
    ...
    if manifest_override and not os.path.isfile(manifest_override):
        print(f"❌ 指定的清单文件不存在: {manifest_override}")
        sys.exit(1)

    decode_all(scan_dir, output_dir, num_workers, manifest_override=manifest_override)
